...
calculate_mh_stats <- function(df, items_to_test, matching_items, ref_grp, foc_grp) {
  ...
  res_list <- lapply(items_to_test, function(item) {
    # Subsetting only essential columns: current item and grouping column.
    # This avoids full dataframe copying and reduces memory allocation.
    item_df <- df[, c(item, "GROUP"), drop = FALSE]
