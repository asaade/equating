...
plot_score_comparison_violin <- function(final_scores, config = NULL) {
  ...
  # Calcular estadísticos descriptivos para mostrar como texto
  ...
  # --- 2. CÁLCULO DE PORCENTAJES (IMPACT DATA) ---
  props <- plot_data |>
    dplyr::group_by(Form_Label, Nivel_Factor) |> # Agrupar por Forma base
  ...
  # Posicionamiento en mediana
  pos_ref <- plot_data |>
    dplyr::group_by(Form_Label, Nivel_Factor) |>
    dplyr::summarise(Y_Pos = mean(!!sym(col_eq), na.rm = TRUE), .groups = "drop")

  # Unir con stats_summary para tener el eje X correcto después
  ann_text <- dplyr::inner_join(props, pos_ref, by = c("Form_Label", "Nivel_Factor")) |>
    dplyr::inner_join(stats_summary, by = c("Form_Label" = "Form_Label"))

  # --- 3. CORTES (LÍNEAS HORIZONTALES) ---
  perf_levels <- config$scoring$performance_levels
  ...
  ref_labels <- if (is.list(perf_levels)) unlist(lapply(perf_levels, function(x) x$label)) else sapply(perf_levels, function(x) x$label)
  ref_cuts <- as.numeric(unlist(ref_cuts))
  min_y <- sort(unique(ref_cuts))[1]
  cuts_viz <- sort(unique(ref_cuts))[-1]

  # --- 4. CONSTRUCCIÓN DEL GRÁFICO ---
  p <- ggplot() +
    # A. Violín (Blanco y negro puro)
    geom_violin(
      data = plot_data, aes(x = Label_Stats, y = !!sym(col_eq)),
      color = "black", size = 0.5, scale = "width", width = 0.7
    ) +

    # B. Boxplot (Gris claro para contraste)
    geom_boxplot(
      data = plot_data, aes(x = Label_Stats, y = !!sym(col_eq)),
      width = 0.15, fill = "gray90", outlier.shape = 1, outlier.size = 1, alpha = 0.8
    )

  # D. Líneas de Corte (Restauradas)
  if (!is.null(cuts_viz) && length(cuts_viz) > 0) {
    p <- p + geom_hline(yintercept = cuts_viz, linetype = "dashed", color = "gray20", linewidth = 0.4)
  }

  # E. Etiquetas de Porcentaje (Texto negro simple)
  p <- p + geom_label(
    ...
    theme_audit_precision() +
    theme(
      axis.text.x = element_text(size = 9, face = "bold"), # Énfasis en los números
      panel.grid.major.x = element_blank() # Limpiar fondo vertical
    )
